fillTable :: [Integer] -> Integer
fillTable [x,0] = 0
fillTable [x,m] = x + fillTable [x,m-1]
...
permutation :: String -> Int -> Int -> String
permutation tbl 0 _ = tbl
permutation tbl n m = 
    let
        tmp_m = m
        tmp_n = n - 1
        ...
        tbl' = take (tmp_m-1) tbl ++ [temp_x] ++ [temp_z] ++ drop (m+1) tbl
    in
        permutation tbl' (n-1) (m-1)
...
triangularNumber [n, m] =
  let
    ...
    numbers1 = permutation numbers (n-1) m
  in
    numbers1
